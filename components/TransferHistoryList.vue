      ...
      <div class="select-wrap ml-2 mr-4">
        <v-autocomplete
          ...
          :menu-props="{
            ...
            'max-width': 120,
            nudgeBottom: 6,
            contentClass: 'ps-dropdown-asset'
          }"
          cache-items
          flat
          hide-no-data
        ...
        >
          <span slot="item" slot-scope="{ item }" :title="item.text">{{
            item.text
          }}</span>
        </v-autocomplete>
      </div>
      ...
      <template slot="items" slot-scope="props">
        <tr :class="{ 'expand-row': props.expanded }">
          <td class="text-xs-center">
            <v-icon>{{
              `ic-${props.item.type === 'send' ? 'outcome' : 'income'}`
            }}</v-icon>
          </td>
          <td class="text-xs-left">
            {{ props.item.time | date('DD/MM/YYYY HH:mm:ss') }}
          </td>
          <td class="text-xs-right">
            {{ props.item.type === 'send' ? '-' : '+' }}&nbsp;{{
              (props.item.amount / Math.pow(10, props.item.precision))
                | floorDigits(props.item.precision)
            }}
            <asset-pairs :asset-id="props.item.asset" />
          </td>
          <td class="text-xs-right">
            <div class="acct-name" :title="props.item.to_name">
              {{ props.item.to_name }}
            </div>
          </td>
          <td class="text-xs-right">
            <div class="acct-name" :title="props.item.from_name">
              {{ props.item.from_name }}
            </div>
          </td>
          <td class="text-xs-right">
            <template v-if="props.item.expiration">{{
              props.item.expiration | date('DD/MM/YYYY HH:mm:ss')
            }}</template>
          </td>
          <td class="text-xs-right">
            <a
        ...
        <div
          :class="{
            'margin-auto':
              ...
              !memokey ||
              memokey === 'empty' ||
              memoList[props.index] === null
          }"
        >
          <template v-if="islocked">
            <cybex-btn tiny class="unlock-btn" @click="$toggleLock()">{{
              $t('button.unlock')
            }}</cybex-btn>
          </template>
          <template v-else>
            <div
              v-if="
                memokey && memokey !== 'empty' && memoList[props.index] !== null
              "
            >
