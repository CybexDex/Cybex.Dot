...
export default {
  ...
  watch: {
    baseId: {
      ...
      handler(newVal, oldVal) {
        if (newVal) {
          this.baseName = this.checkNameById(newVal)
        }
      }
    },
    quoteId: {
      ...
      handler(newVal, oldVal) {
        if (newVal) {
          this.quoteName = this.checkNameById(newVal)
        }
      }
    },
    assetId: {
      ...
      handler(newVal, oldVal) {
        if (newVal && !this.baseId && !this.quoteId) {
          this.assetName = this.checkNameById(newVal)
        }
      }
    }
  },
  methods: {
    ...
    isInCustomAsset(name) {
      ...
      return !((isInWhitelist || isWhitePrefix) && !isGameAsset)
    },
    checkNameById(id) {
      let name = id
