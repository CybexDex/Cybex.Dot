...
export default {
  ...
  mounted() {
    this.isReady = false
    const datafeed = this.initKlineData()
    const widgetOptions = {
      // debug: true,
      symbol: this.symbol,
...
.small-size {
  .tv-chart-container {
    height: calc(60% - #{$small-activity-height});
  }
}

.tv-chart-container {
  position: relative;
  min-height: calc(#{$k-line-min-height} + #{$k-line-toolbar-height});
  height: calc(60% - #{$activity-height});
...
#tv_chart_container {
  // min-height: k-line-min-height
  height: calc(100% - #{$k-line-toolbar-height});
}
